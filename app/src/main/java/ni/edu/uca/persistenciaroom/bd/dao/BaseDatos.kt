...
@Database(
    entities = [UsuariosEntity::class], version = 1
)
    ...
    companion object {
        ...
        fun getDataBase(context: Context): MainBaseDatos {
            synchronized(this) {
                ...
                if (instance == null) {
                    instance = Room.databaseBuilder(context.applicationContext,MainBaseDatos::class.java,
                    "usuario_main_db").fallbackToDestructiveMigration().build()
                    INSTANCE = instance }
                return instance
            }
        }
