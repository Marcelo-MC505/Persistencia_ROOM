...
import androidx.lifecycle.ViewModelProvider import androidx.navigation.fragment.findNavController
...
class ActualizarFragment : Fragment() {
    ...
    fBinding = FragmentActualizarBinding.inflate(layoutInflater)
        ...
        with(fBinding) { TxtUserName.setText(args.currentUsuario.nombreUsuario)
    ...
    private fun GuardarCambios() {
        ...
        val user = UsuariosEntity(args.currentUsuario.idUsuario, userName, pass, email, nombres, apellidos)
        viewModel.actualizarUsuario(user)
        Toast.makeText(requireContext(), "Registro actualizado", Toast.LENGTH_LONG).show()
        findNavController().navigate(R.id.actualizarLista)
    }
    ...
    private fun eliminarUsuario() {
        ...
        alerta.setPositiveButton("Si") { _, _ -> viewModel.eliminarUsuario(args.currentUsuario)
